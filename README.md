...
- `禁用` / `启用`：控制某条线路是否参与输出。
- `测试播放`：打开网页播放器测试当前线路。远程 `.m3u8`、`http://.../rtp/...`、`http://.../udp/...` 或 `.ts` 这类直播流会由 VPS 里的 FFmpeg 临时转为 HLS 后预览，避免浏览器跨域和编码兼容问题，启动会多等几秒但更适合连续播放。FFmpeg 默认等待 25 秒生成首个清单，可用 `HLS_START_TIMEOUT_MS` 调整毫秒数；测试页关闭后，FFmpeg 默认空闲 30 秒会自动停止，可用 `HLS_IDLE_TIMEOUT_MS` 调整毫秒数，设为 `0` 可禁用自动停止。真正的 `rtp://`、`udp://`、`rtsp://` 仍然通常需要电视端验证。
