...
- `分组`：手动设置频道所属分组；留空时只显示在 `全部频道`，填写后还会出现在对应自定义分组里。
- `置顶` / `上移` / `下移`：辅助调整同序号或未编号频道的默认顺序。
- `设为默认`：把该线路放到这个频道的第一个源。
- `禁用` / `启用`：控制某条线路是否参与输出。

`/live.txt` 会固定先输出 `全部频道` 分组，并按网页排序号显示所有未隐藏频道；后面再输出你手动填写的自定义分组。原始采集源里的默认分组不会再直接用于 `/live.txt`。
...
- `PUT /api/channels/:channelId/override`：保存频道隐藏、排序号、自定义分组、默认线路、禁用线路。
