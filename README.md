...
- `禁用` / `启用`：控制某条线路是否参与输出。
- `测试播放`：打开网页播放器测试当前线路。浏览器通常支持 `.m3u8`、`.mp4` 等格式；`http://.../rtp/...`、`http://.../udp/...` 或 `.ts` 这类 MPEG-TS/组播代理流会由 VPS 里的 FFmpeg 临时重封装为 HLS 后预览，启动会多等几秒但更适合连续播放。真正的 `rtp://`、`udp://`、`rtsp://` 仍然通常需要电视端验证。
