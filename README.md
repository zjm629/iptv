...
- 网页可设置某个频道的默认线路。
- 网页可禁用/启用某条线路。
- 网页可打开测试播放器，远程 HLS、MPEG-TS/组播代理流会通过 FFmpeg 临时转成低码率 HLS 预览。
...
- `禁用` / `启用`：控制某条线路是否参与输出。
- `测试播放`：打开网页播放器测试当前线路。远程 `.m3u8`、`http://.../rtp/...`、`http://.../udp/...` 或 `.ts` 这类直播流会由 VPS 里的 FFmpeg 临时转为低码率 HLS 后预览，避免浏览器跨域、编码兼容和源站码率过高导致的卡顿。FFmpeg 默认等待 25 秒生成首个清单，可用 `HLS_START_TIMEOUT_MS` 调整毫秒数；测试页关闭后，FFmpeg 默认空闲 30 秒会自动停止，可用 `HLS_IDLE_TIMEOUT_MS` 调整毫秒数，设为 `0` 可禁用自动停止。真正的 `rtp://`、`udp://`、`rtsp://` 仍然通常需要电视端验证。
